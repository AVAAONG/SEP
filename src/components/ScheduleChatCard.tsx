...
type ScheduleChatCardProps = {
    chats: ChatsWithAllData[];
};

const ScheduleChatCard: React.FC<ScheduleChatCardProps> = ({ chats }) => {
    ...
    // const deleteWorkshop = async (chatId: string) => {
    //     const workshop = chats.find((chat) => chat.id === chatId);
    //     workshop?.calendar_ids.map(async (eventId) => {
    //         await deleteCalendarEvent(CHAT_CALENDAR_ID, eventId);
    //     });
    //     await deleteWorkshopFromDatabase(chatId);
    //     if (workshop?.modality === 'ONLINE') {
    //         if (workshop.platform === 'ZOOM') {
    //             await deleteZoomMeeting(chatId);
    //         }
    //     }
    // };
    return (
        <div className="flex flex-col w-full items-center">
            ...
            >
                {chats.map((workshop: ChatsWithAllData, index) => {
                    const {
                        title,
                        id,
                        avalible_spots,
                    ...
                    return (
                        ...
                        >
                            <div className="w-full flex justify-between gap-2 items-center">
                                ...
                                <div className="flex-1 min-w-0 text-center">
                                    <p className="text-xs">Nivel {level.toString()}</p>
