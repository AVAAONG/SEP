...
type ScheduleChatCardProps = {
    activities: ChatsWithAllData[];
};

const ScheduleChatCard: React.FC<ScheduleChatCardProps> = ({ activities }) => {
    const [groupSelected, setGroupSelected] = useState<string[]>([]);
    const [buttonIsDisabled, setbuttonIsDisabled] = useState(false);
    const [selectedActivity, setSelectedActivity] = useState<ChatsWithAllData | null>(null);
    ...
    const editWorkshop = async (chatId: string) => {
        router.push(`?editChatId=${chatId}`);
    };

    const deleteActivity = async (activityId: string) => {
        const activity = activities.find((activity) => activity.id === activityId);
        activity?.calendar_ids.map(async (eventId) => {
        ...
        //         await deleteZoomMeeting(activityId);
        //     }
        // }
    }
    return (
        <div className="flex flex-col w-full items-center">
            ...
            <ScrollShadow hideScrollBar className="w-full h-[550px]">
                ...
                >
                    {activities.map((workshop: ChatsWithAllData, index) => {
                        const {
                            title,
                            id,
                            avalible_spots,
                                        ...
                                        <p className="text-xs  ">
                                                ...
                                                content={<div className='flex flex-col gap-2'>
                                                    {
                                                        speaker.map((speaker, index) => {
                                                            return (
                                                                <div key={index}>
                                                                    {speaker.first_names.split(' ')[0] + ' ' + speaker.last_names.split(' ')[0]}
                                                                </div>
                                                            )
                                                        })
                                                    }
                                            ...
                                            >
                                                ...
                                                <Button className="bg-transparent p-0 h-unit-5" variant="flat">
                                                    Por: {speaker[0].first_names.split(' ')[0] + ' ' + speaker[0].last_names.split(' ')[0]}
                                                </Button>
                                            </Tooltip>
                                        </p>
                                    </div>
                                                ...
                                                return (
                                                    <>
                                                        <div className="flex flex-col gap-4 items-center justify-center w-full">
                                                            <p>La actividad <span className='font-medium italic'>{selectedActivity!.title}
                                                            </span> pautada para el dia <span className='font-medium '>{new Date(selectedActivity!.start_dates[0]).toLocaleDateString('es-ES',
                                                                { day: 'numeric', month: 'long', year: 'numeric' })}</span> a las <span className='font-medium '>{new Date(selectedActivity!.start_dates[0]).toLocaleTimeString('es-ES', {
                                                                    hour: '2-digit',
                                                                    minute: '2-digit',
                                                                    hour12: true
                                                                })}</span> sera eliminada de forma permanente</p>
                                                            <p>En el caso de querer agendar una actividad con los mismos datos deberas crearla nuevamente</p>
                                                        </div>
                                                    </>
                                                );
                                            }}
