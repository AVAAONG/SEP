...
const ActivityScholarActions: React.FC<ActivityPanelInfoProps> = ({ scholars, attendanceId, activityId, kindOfActivity, isButtonDisabled }) => {
    const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
    const [selectedScholar, setSelectedScholar] = useState<React.Key | undefined>()
    const handleCeaseSpot = async () => {
        if (kindOfActivity === 'workshop') await ceaseSpotInWorkshop(attendanceId, scholars[0].id, activityId, selectedScholar!.toString())
        else if (kindOfActivity === 'chat') await ceaseSpotInChat(attendanceId, scholars[0].id, activityId, selectedScholar!.toString())
    }
                ...
                Content={() => (
                    <div className="flex flex-col gap-4">
                        <p className="text-sm list-disc space-y-sm w-full">
                            Al cancelar tu inscripción, cederás tu cupo a otro becario, y tu asistencia dentro de la actividad pasara a contarse como "Cancelado".
                        </p>
                        <p className="font-medium">Si te arrepientes, no podrás volver a inscribirte. 👀</p>
                        <p className="text-sm list-disc space-y-sm w-full">
                            Si estas seguro de cancelar tu inscripción, primero busca a un becario que pueda ocupar tu lugar en la actividad.
                        </p>
                        <Autocomplete
                        ...
                        >
                            {(scholar) => (
                                <AutocompleteItem
                                    key={scholar.id}
                                    textValue={`${scholar.first_names.split(' ')[0]} ${scholar.last_names.split(' ')[0]}`}
                                >
