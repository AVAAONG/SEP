  ...
  return (
    <Accordion variant="splitted">
      {scholarInProbation.program_information.probation.map((probationInfo, index) => {
        const title =
          probationInfo.kind_of_probation === 'PROBATION_II' ? 'Probatorio II' : 'Probatorio I';
              ...
              <div className="flex flex-col md:flex-row gap-4 md:gap-10">
                ...
                <div>
                  ...
                  <p>
                    ...
                    {new Date(probationInfo.next_meeting).toLocaleDateString('es-ES', {
                      month: 'long',
                      day: 'numeric',
                      year: 'numeric',
                    })}
                  ...
                  {Object.keys(probationInfo.done_at_the_moment).map((key, index) => {
                    return (
                      <div key={index} className="px-2 flex flex-col gap-1">
                        ...
                        <p className="">{probationInfo.done_at_the_moment[key]}</p>
                      </div>
                    );
                  })}
                </div>
                  ...
                  {Object.keys(probationInfo.agreement).map((key, index) => {
                    return (
                      <div key={index} className="px-2 flex flex-col gap-1">
                        ...
                        <p>{probationInfo.agreement[key]}</p>
                      </div>
                    );
                  })}
                </div>
