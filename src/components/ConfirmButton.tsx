...
const AddWorkshopButton: React.FC<Props> = ({ scholarId, data }) => {
    ...
    const handleAddWorkshop = async () => {
        ...
        const confirm = window.confirm(`Are you sure you want to add ${selectedWorkshop.title} to this scholar?`);
        if (confirm) {
    ...
    return (
        <>
            ...
            {showWorkshops && (
                <div>
                    <input type="text" placeholder="Search workshops" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    <ul>
