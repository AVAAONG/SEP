import { getDate } from '@/lib/calendar/calendar';
import { WorkshopDates, WorkshopSpeaker, WorkshopTempData } from '@prisma/client';
    ...
    return (
        <form onSubmit={handleSubmit(async (data, event) => await scheduleWorkshop(data, event!))} className="grid gap-6 md:grid-cols-2 md:grid-rows-2 caret-green-500 text-slate-300 w-full">
            ...
            <div >
                ...
                <input  {...register("endHour")} type={'date'} id={"Fecha"} required={true} min={getDate()} />
            </div>

            <div >
                <label className="block mb-2 text-sm m-l-1 font-medium  text-slate-400"> Hora de inicio</label>
                <input  {...register("date")} type={'date'} id={"Hora de inicio"} required={true} min={getDate()} />
            </div>

            <div >
                <label className="block mb-2 text-sm m-l-1 font-medium  text-slate-400"> Hora de cierre</label>
                <input  {...register("startHour")} type={'date'} id={"Hora de cierre"} required={true} min={getDate()} />
            </div>
            ...
            <div >
                ...
                <input {...register("spots")} type={"number"} id="first_name" required />
            </div>
                    ...
                    {WORKSHOP_YEAR.map((input) => {
                        return (
                            <div className="flex flex-row items-center mr-4" key={input}>
                                <input {...register("avaaYear")} type="checkbox" value={input} id='year' className='w-4 h-4 bg-emerald-900  accent-green-500 text-emerald-600 border-gray-300 focus:ring-0 ' />
