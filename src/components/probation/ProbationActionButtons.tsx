...
const ProbationActionButtons: React.FC<ProbationAccordionProps> = ({ probation, scholarData }) => {
    ...
    openConfirmation(
      {
        ...
        message: '¿Está seguro que desea borrar este estatus de probatorio? Esta acción no se puede deshacer.',
        confirmText: 'Borrar',
        cancelText: 'Cancelar'
      },
      async () => toast.promise(deleteProbation(probation.id, scholarData.id),
        {
  ...
  const handleEndScholarProbation = () => {
    openConfirmation(
      ...
      async () => toast.promise(endScholarProbation(probation.id, scholarData.id),
        {
  ...
  return (
    <>
      ...
      <Button onPress={handleDelete}
        color={probation.kind_of_probation === 'PROBATION_I' ? 'warning' : 'danger'} radius="sm" variant="ghost">
        Borrar
      </Button>
      <Button onPress={modalActions.onOpen} color={probation.kind_of_probation === 'PROBATION_I' ? 'warning' : 'danger'} radius="sm" variant="ghost">
        Editar
      </Button>
      <Button isDisabled={probation.ending_date !== null} onPress={handleEndScholarProbation}
        color={probation.kind_of_probation === 'PROBATION_I' ? 'warning' : 'danger'} radius="sm" variant="ghost">
