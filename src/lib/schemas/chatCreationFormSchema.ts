...
const chatCreationFormSchema = z.object({
    title: z.string().min(1, { message: 'El titulo no puede estar vacio' }).trim(),
    dates: z.object({
        date: z.string().min(1, { message: 'Debes especificar la fecha' })
    ...
    }).array().min(1, { message: 'Debes especificar al menos una' }),
    ...
    avalible_spots: z.coerce.number({
        errorMap: (issue, _ctx) => {
            switch (issue.code) {
                default:
