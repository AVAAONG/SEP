...
const page = async ({
    ...
    return (
        <div className="space-y-6 text-primary p-8">
            <div className="flex flex-col space-y-3 items-center"><span className="pipe w-fit text-green-500 text-center font-semibold">Actividad formativa</span>
                <h1 className="text-2xl font-semibold leading-none tracking-tight text-center text-primary md:text-5xl">{title}</h1>
            ...
            <section className="space-y-3">
                ...
                <div className="space-y-2">
                    <div className="-ml-1.5 space-y-2 border-l-2 border-green-500 pl-1.5 sm:-ml-3 sm:pl-3">
                        <div className="space-y-sm">
                            ...
                            <p className="text-base font-semibold">{new Date(dates[0].start_date).toLocaleDateString()}</p>
                        </div>
                        <div className="space-y-sm">
                            <h3 className="text-sm leading-6 text-secondary">Hora:</h3>
                            <p className="text-base font-semibold">{new Date(dates[0].start_date).toLocaleTimeString()}</p>
                        </div>
                        <div className="space-y-sm">
                            <h3 className="text-sm leading-6 text-secondary">{modality === "VIRTUAL" ? "Plataforma" : "Lugar"}</h3>
                            <p className="text-base font-semibold capitalize">{platform} ({modality})</p>
                        </div>
                    </div>
                    ...
                    <p className="ml-2 list-disc space-y-sm">
                        {description}
                    </p>
                </div>
                <div className="w-full space-y-3">
                    ...
                    <div className="flex flex-row items-center space-x-2">
                        <div className="h-9 w-9 shrink-0"><img alt="Gabriel Madureira" loading="lazy" src="https://planetscale-images.imgix.net/build/_assets/gabriel-madureira-JI6E4AAH.jpeg?auto=compress%2Cformat" className="max-h-[72px] overflow-hidden rounded-full" width="72" height="72" /></div>
                        <div className="space-y-sm">
                            <div>
