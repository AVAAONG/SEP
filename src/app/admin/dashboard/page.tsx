...
const fetchEvents = async () => {
  ...
  const formatedVolunteerEvents = createEventObject(
    volunteerEvents!,
    CALENDAR_IDS[0].eventColor,
    CALENDAR_IDS[0].textColor
  );
  calendarEvents.push(...formatedVolunteerEvents);

  const chatsEvents = await getCalendarEvents(CALENDAR_IDS[1].calendarId);
  const formatedChatsEvents = createEventObject(
    chatsEvents!,
    CALENDAR_IDS[1].eventColor,
    CALENDAR_IDS[1].textColor
  );
  calendarEvents.push(...formatedChatsEvents);

  const workshopsEvents = await getCalendarEvents(CALENDAR_IDS[2].calendarId);
  const formatedWorkshopsEvents = createEventObject(
    workshopsEvents!,
    CALENDAR_IDS[2].eventColor,
    CALENDAR_IDS[2].textColor
  );
